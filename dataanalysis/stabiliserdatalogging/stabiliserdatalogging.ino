...
void loop() {
  ...
  while (test > threshold){
    ...
    for (int i = 0; i < avgnum; i++) {
      ...
      float min_s = testArray[0];
      for ( int i = 0; i < 1000; i++ ) {
        if ( testArray[i] > max_s ) {
          max_s = testArray[i];
        }
        if ( testArray[i] < min_s ) {
          min_s = testArray[i];
        }
      }
      maxsumm += max_s;
    ...
    test =  maxsumm/avgnum;
    testtwo =  minsumm/avgnum;
    tau += avgnum * 1000 * scantime;
    Serial.println("op");
    Serial.println(test);
    Serial.println("c");
  ...
  while (test < threshold) { //recovery mode
    ...
    while (test < threshold) { // run downwards in current by 0.1 until lock is reestablished
      ...
      testtwo =  minsumm/avgnumtwo;
      tau += avgnumtwo * 1000 * scantime;
      Serial.println("op");
      Serial.println(test);
      Serial.println("c");
      Serial.println(testtwo);
